...
export default function CollectionsPage() {
  ...
  const openModal = (collection?: any) => {
    if (collection) {
      ...
      setFormData({ name: collection.name, end_use: collection.end_use, active: collection.active });
    } else {
      setEditingId(null);
  ...
  return (
    <div className="space-y-6">
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={4} className="text-center text-slate-500">No collections found.</TableCell></TableRow>
            ) : (
              collections.map((col) => (
                <TableRow key={col.id}>
                  ...
                  <TableCell className="text-right">
                    <Button variant="ghost" size="icon" onClick={() => openModal(col)}><Edit className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon" className="text-red-500" onClick={() => handleDelete(col.id)}><Trash2 className="h-4 w-4" /></Button>
                  </TableCell>
                </TableRow>
              ))
            )}
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Edit Collection' : 'New Collection'}>
